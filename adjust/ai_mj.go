...
func ReplaceBaiWithLZ(chiCard []*ChiCard, aiCards []AICard, lzCard int32) {
...
func ReplaceLZWithBai(chiCard []*ChiCard, aiCards []AICard, lzCard int32) {
	for _, v := range chiCard {
		if v.CardId == lzCard && v.CardType == MJ_CHI_CHI {
...
func ConvertSliceToAICard(holdCards []int32, card int32, lzCard int32) ([]AICard, int32) {
...
func GetCardValue(rule IMahjong, holdCards []int32, moCard int32) []CardValue {
	...
	if rule.CanReplaceForBai() == true {
		for i := 0; i < len(valCards); i++ {
			if valCards[i].Card == rule.GetLaiziCard() {
...
func removeSolKeSeq(rule IMahjong, cards []int32) []int32 {
	lzCard := rule.GetLaiziCard()
	for i := 0; i < len(cards); i++ {
		if cards[i] == lzCard {
			cards[i] = MAHJONG_LZ
		} else if rule.CanReplaceForBai() == true && cards[i] == MAHJONG_MASK*COLOR_OTHER+MAHJONG_BAI {
			cards[i] = lzCard
		}
	}
	sort.Slice(cards, func(i, j int) bool { return cards[i] < cards[j] })
	...
	for i := 0; i < len(cards); i++ {
		if cards[i] == MAHJONG_LZ {
			cards[i] = lzCard
		} else if rule.CanReplaceForBai() == true && cards[i] == lzCard {
...
//获取AI
func GetAIForMJ() *AIMahjong {
	return &AIMahjong{}
}

//设置规则
func (ai *AIMahjong) SetRule(rule IMahjong) {
	ai.rule = rule
}

//设置所有牌
func (ai *AIMahjong) SetAllCard(holdCards map[int32][]int32, chiCards map[int32][]*ChiCard, usedCards map[int32][]int32, restCards []int32) {
...
//设置级别
func (ai *AIMahjong) SetLevel(level int32) {
	ai.level = level
...
*/
func (ai *AIMahjong) getCardRemainNum(cards []int32, chiCards []*ChiCard, index int32, card int32) int32 {
	rNum := int32(4)
	if card == MAHJONG_ANY {
		return 10000
	} else if card == ai.rule.GetLaiziCard() { //听牌中必然含癞子牌，所以可统一返回，随后再考虑翻牌
		return rNum
	}
...
//获取想要的下一张牌
func (ai *AIMahjong) GetNextCards(index int32) []int32 {
...
func (ai *AIMahjong) GetUnusedCards(index int32) []int32 {
	...
	for i := 0; i < len(aiCards); i++ {
		...
		lackColor = util.RemoveArrayElem(lackColor, aiCards[i].Card/MAHJONG_MASK)
	...
	unusedC = util.RemoveArrayElem(unusedC, ai.rule.GetLaiziCard())
	sort.Slice(unusedC, func(i, j int) bool { return unusedC[i] < unusedC[j] })
	return unusedC
}

//获取玩家需要的牌
func (ai *AIMahjong) GetNeedCards(index int32) []int32 {
	needC := []int32{}
	...
	if len(needC) == 0 {
		for _, v := range aiCards {
			if v.Card == ai.rule.GetLaiziCard() {
				continue
			}
...
//托管出牌
func (ai *AIMahjong) GetCardForMandate(index int32, moCard int32) int32 {
	if *config.FLAG_DEBUG == 1 || moCard == 0 || moCard == ai.rule.GetLaiziCard() {
...
//机器人出牌
func (ai *AIMahjong) GetCardForRobot(index int32, moCard int32) int32 {
...
//随机出牌
func (ai *AIMahjong) getCardForRand(holdCards []int32, moCard int32) int32 {
...
//权重出牌
func (ai *AIMahjong) getCardForWeight(holdCards []int32, moCard int32) int32 {
...
//向听数出牌
func (ai *AIMahjong) getCardForTingNum(holdCards []int32, moCard int32, index int32, chiCards []*ChiCard) int32 {
	cards := make([]int32, len(holdCards))
	copy(cards, holdCards)
	if moCard > 0 {
	...
	for _, vc := range valCards {
		cards = util.RemoveArrayElem(cards, vc.Card)
...
//托管吃牌
func (ai *AIMahjong) GetChiForMandate(index int32, moCard int32, chiCards []*ChiCard) *ChiCard {
...
//机器人吃牌
func (ai *AIMahjong) GetChiForRobot(index int32, moCard int32, chiCards []*ChiCard) *ChiCard {
	if len(chiCards) == 0 || chiCards[0] == nil {
		return nil
	}
	chiCard := NewChiCard()
	*chiCard = *chiCards[0]
	...
	if ai.level == ROBOT_LEVEL_NOOB { //菜鸟级机器人在一上一听阶段截住吃和碰
		if chiCard.CardType < MJ_CHI_HU || chiCard.CardType > MJ_CHI_GANG_AN {
			_, tings := ai.rule.CheckNTing(ai.chiCards[index], ai.holdCards[index], 1, false)
			if len(tings) > 0 {
				chiCard.CardType = MJ_CHI_PASS
			}
		...
		** 碰和吃的独立依赖于特定规则，例如碰可能会破坏将或七对，吃可能会破坏全不靠
		...
		if moCard > 0 {
			...
			preCards = util.RemoveArrayElem(preCards, playC)
		}
		...
		for _, v := range newChiCards {
			...
			if v.CardType == MJ_CHI_GANG_AN || v.CardType == MJ_CHI_GANG {
				curCards = util.RemoveArrayElem(curCards, v.CardId)
				curCards = util.RemoveArrayElem(curCards, v.CardId)
				curCards = util.RemoveArrayElem(curCards, v.CardId)
...
//获取独立的杠
func (ai *AIMahjong) getAloneGang(index int32, moCard int32, chiCards []*ChiCard) *ChiCard {
	if len(chiCards) > 0 && chiCards[0] != nil &&
		...
		for _, v := range chiCards {
			card := v.CardId
			if ai.rule.CanReplaceForBai() == true && v.CardId == COLOR_OTHER*MAHJONG_MASK+MAHJONG_BAI {
				card = ai.rule.GetLaiziCard()
			}
...
//移除单张牌
func (ai *AIMahjong) removeSingleCard(chiCards []*ChiCard, cards []int32) []int32 {
	singleN := 0
	valCards := GetCardValue(ai.rule, cards, 0)
	...
	if singleN > 3 {
		for i := 0; i < singleN-singleN%3; i++ {
			cards = util.RemoveArrayElem(cards, valCards[i].Card)
		}
	}
	return cards
}
...
//获取主牌的向听数
func (ai *AIMahjong) getMainCardTingNum(chiCards []*ChiCard, holdCards []int32) int32 {
	cards := make([]int32, len(holdCards))
	copy(cards, holdCards)
	sort.Slice(cards, func(i, j int) bool { return cards[i] < cards[j] })
